function PDI = Rawdata2MATnew_V4_droplet(datapath, savepath)
...
%
    ...
    if isfield(cfg.stimuli, 'droplet_stimuli')
        ...
        if exist(csvPath, 'file')
            ...
            % -------------------------------------------------------------
            % A. EXTRACT HARDWARE TTL TIMESTAMPS (Pure NIDAQ Clock)
            % -------------------------------------------------------------
            ...
            % -------------------------------------------------------------
            % B. ALIGN CSV 'DROP' EVENTS VIA CHANNEL 5 HARDWARE START
            % -------------------------------------------------------------
            ...
            % -------------------------------------------------------------
            % C. PACK SEPARATE PREDICTOR TABLES INTO PDI.stimInfo
            % -------------------------------------------------------------
