function func_preprocessing(anat_analysis_path, func_analysis_path)
% FUNC_PREPROCESSING Preprocess functional ultrasound imaging (fUSI) data.
%
% Syntax:
%   fonduta.preprocessing.func_preprocessing(anat_analysis_path, func_analysis_path)
%
% Inputs:
%   anat_analysis_path - Path to anatomical Data_analysis directory
%   func_analysis_path - Path to functional Data_analysis directory
%
% Example:
%   fonduta.preprocessing.func_preprocessing(...
%       '/data03/fUSI/Data_analysis/sub-01/ses-231215/run-113409-anat', ...
%       '/data03/fUSI/Data_analysis/sub-01/ses-231215/run-115047-func');

if nargin < 2
    error('func_preprocessing:MissingInput', 'Both anat_analysis_path and func_analysis_path must be provided.');
end

% Validate Anatomical Directory & Files
anatFile = fullfile(anat_analysis_path, 'anatomic.mat');
transFile = fullfile(anat_analysis_path, 'Transformation.mat');

if ~exist(anatFile, 'file')
    error('func_preprocessing:MissingFile', 'Missing "anatomic.mat" in: %s', anat_analysis_path);
end
if ~exist(transFile, 'file')
    error('func_preprocessing:MissingFile', 'Missing "Transformation.mat" in: %s', anat_analysis_path);
end

% Validate Functional Directory & File
pdiFile = fullfile(func_analysis_path, 'PDI.mat');

if ~exist(pdiFile, 'file')
    error('func_preprocessing:MissingFile', 'Missing "PDI.mat" in: %s', func_analysis_path);
end

% Print Verified Target Paths
fprintf('All required files verified:\n');
fprintf('  Anatomic:       %s\n', anatFile);
fprintf('  Transformation: %s\n', transFile);
fprintf('  Functional PDI: %s\n', pdiFile);
...
[PDI, anatomic, Transf, atlas] = load_anat_func_and_atlas(anat_analysis_path, func_analysis_path);
