...
for isub = 1:nSubs
    ...
    try
        ...
        tc_sub_mat      = zeros(win_len, nROI);
        baseline_len    = k_before * step;
        ...
        for r = 1:nROI
            ...
            % --- Extract trial-averaged peristimulus time course ---
            epoch_mat = zeros(win_len, nTrials);
            for tr = 1:nTrials
                tf = trial_frames(tr, :);
                if any(tf < 1 | tf > T_safe); continue; end
                epoch = y(tf);
                baseline = mean(epoch(1:baseline_len));
                epoch_mat(:, tr) = epoch - baseline;
            end
            tc_sub_mat(:, r) = mean(epoch_mat, 2);
        ...
        for r = 1:nROI
            ...
            if ~isfield(regional_hrf, field)
                ...
                regional_avg.(field).tc    = tc_sub_mat(:,r);
                ...
                regional_avg.(field).tc    = [regional_avg.(field).tc, tc_sub_mat(:,r)];
            end
        end
...
save(out_fname, ...
    'regional_hrf', 'regional_avg', 'atlas', 'model_name', ...
